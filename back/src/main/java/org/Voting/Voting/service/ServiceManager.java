...
import org.Voting.Voting.config.SystemConfig;
...
public class ServiceManager {
  ...
  /**
   * @notice: must use @Qualifier("VotingService") with @Autowired to get this Bean
   */
  @Bean("VotingService")
  public Map<String, VotingService> initVotingServiceManager() throws Exception {
    Map<String, VotingService> serviceMap = new ConcurrentHashMap<>(this.hexPrivateKeyList.size());
    for (int i = 0; i < this.hexPrivateKeyList.size(); i++) {
    	...
    	votingService.setAddress(this.config.getContract().getVotingAddress());
    	votingService.setClient(this.client);
    	...
    	votingService.setTxProcessor(txProcessor);
    	serviceMap.put(userAddress, votingService);
    }
    log.info("++++++++VotingService map:{}", serviceMap);
    return serviceMap;
  }
}
